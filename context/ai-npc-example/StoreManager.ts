  ...
  for(let i = 0; i < StoreData.InventoryLists.length; i++)
  {
    ...
    for(let j = 0; j < StoreData.InventoryLists[i].length; j++)
    {
      ...
      if(metaData)
      {
        ...
        items.push({... StoreData.InventoryLists[i][j], price : price});
      }
    }
    NpcManager.setStoreData(i, items, player);
  }
}
...
function PurchaseHandler(storeId: bigint, itemId: string, status: ItemStatus, price: CurrencyType, player: hz.Player)
{
  ...
  if(metaData)
  {
    NpcManager.onPlayerPurchase(player, `a ${metaData.toolId} of type ${metaData.type} for ${itemCost.amount} ${itemCost.name}`); 
